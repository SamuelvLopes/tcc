...
As faixas utilizadas no sistema (\texttt{INITIAL}, \texttt{BASIC}, \texttt{INTERMEDIATE}, \texttt{IMPROVING}, \texttt{ADVANCED}) foram definidas como convenção de classificação para o contexto desta implementação. O objetivo prático foi separar cenários críticos (abaixo de 0,30), básicos (0,30 a 0,49), intermediários (0,50 a 0,69), em evolução (0,70 a 0,89) e avançados (a partir de 0,90), preservando interpretação simples pelos gestores.

Trata-se, portanto, de uma escala heurística de apoio à decisão, adequada à prova de conceito e ao acompanhamento evolutivo interno. Como limitação, reconhece-se que a calibração dos limiares ainda requer validação empírica com maior volume de ciclos e comparação com benchmarks externos.
...
Entre as limitações, destacam-se a necessidade de expansão da cobertura de testes para cenários de tenancy em todos os endpoints, a ausência de comparação experimental com abordagens alternativas e a importância de validação longitudinal em múltiplos contextos organizacionais. Também se reconhece que a metodologia adotada priorizou implementação e validação funcional por prova de conceito, sem definição prévia de métricas estatísticas de efetividade.
